...
class GhostTypeIMEService : InputMethodService() {
    ...
    private fun stylize(text: String): String =
        if (text.isEmpty()) text else UnicodeFonts.transform(this, text)
    ...
    override fun onFinishInput() {
        // v1.10 — wipe the WPM rolling window so the next chat field
        // starts measuring from zero. Without this a user who types fast
        // in WhatsApp then opens a slow Notes field would see a stale
        // 80 WPM hanging in the badge for the first second.
        WpmTracker.reset()
        keyboardView?.refreshWpm()
        super.onFinishInput()
    }
    ...
    private fun handleKey(key: KeyDef) {
        ...
        // v1.10 — Live WPM tracking. Every "productive" keystroke (letter,
        // digit, symbol, space, punctuation) records a timestamp into the
        // rolling 10-second window inside WpmTracker. Backspace is excluded
        // on purpose — corrections shouldn't inflate the speed reading and
        // a held-down delete would otherwise spike to 200+ WPM. The keyboard
        // view's badge is poked here too so the number updates instantly
        // instead of waiting for the next 1 Hz idle tick.
        when (key.type) {
            KeyType.CHAR, KeyType.SPACE, KeyType.PERIOD, KeyType.COMMA -> {
                WpmTracker.recordChar()
                keyboardView?.refreshWpm()
            }
            else -> { /* ignore — backspace, edit-actions, layout switches */ }
        }
        when (key.type) {
            KeyType.BACKSPACE -> {
